...
function constr_list() {
    ...
    for (i = 0; i < QR_icon.length; i++) {
        QR_icon[i].onclick = function() {
            let selec_client = this.parentElement.nextElementSibling.nextElementSibling.nextElementSibling.innerHTML;
            console.log(selec_client);
            let num_shipping = this.parentElement.nextElementSibling.innerHTML;
            console.log(num_shipping);
